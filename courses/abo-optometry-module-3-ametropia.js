class ABOModule3Ametropia extends HTMLElement {
    connectedCallback() {
        this.innerHTML = `
...
<div class="abo-module">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.next-topic').forEach(btn => {
                btn.addEventListener('click', function() {
                    const nextId = this.dataset.next;
                    document.querySelector(\`[data-topic="\${nextId}"]\`).click();
                });
            });

            document.querySelectorAll('.prev-topic').forEach(btn => {
                btn.addEventListener('click', function() {
                    const prevId = this.dataset.prev;
                    document.querySelector(\`[data-topic="\${prevId}"]\`).click();
                });
            });
            ...
            document.querySelectorAll('.submit-quiz').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    questions.forEach(q => {
                        ...
                        feedback.className = 'feedback show ' + (selectedAnswer === correctAnswer ? 'correct' : 'incorrect');
                    ...
                    if (quizId === 'final') {
                        ...
                        document.getElementById('finalScoreText').textContent = \`You scored \${correct} out of \${total} (\${percentage}%)\` + (passed ? ' - PASSED!' : ' - Please review and try again.');
                        
                        this.classList.add('hidden');
                        document.querySelector('.retry-quiz[data-quiz="final"]').classList.remove('hidden');
                        
                        if (passed) {
                            document.getElementById('finalNavButtons').classList.add('hidden');
                            document.getElementById('completionSection').classList.add('show');
                            document.querySelector('[data-topic="final"]').classList.add('completed');
                        }
                    } else {
                        ...
                        updateProgress();
                        document.querySelector(\`[data-topic="\${quizId}"]\`).classList.add('completed');
                    }
                });
            });
            ...
            document.querySelectorAll('.retry-quiz').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    if (quizId === 'final') {
                        ...
                        document.getElementById('finalNavButtons').classList.remove('hidden');
                    } else {
            ...
            // Start final quiz
            const startFinalQuizBtn = document.querySelector('.start-final-quiz');
            if (startFinalQuizBtn) {
                startFinalQuizBtn.addEventListener('click', function() {
                    document.getElementById('finalQuizIntro').classList.add('hidden');
                    document.getElementById('quiz-final').classList.remove('hidden');
                    document.getElementById('finalQuizActions').classList.remove('hidden');
                });
            }
    ...
    initializeModule() {
        const topicTabs = this.querySelectorAll('.topic-tab');
        const topicContents = this.querySelectorAll('.topic-content');
        ...
        topicTabs.forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                this.querySelector('#topic-' + topicId).classList.add('active');
        ...
        // Next/Previous topic buttons
        this.querySelectorAll('.next-topic').forEach(btn => {
            btn.addEventListener('click', () => {
                const nextId = btn.dataset.next;
                this.querySelector(\`[data-topic="\${nextId}"]\`).click();
            });
        });

        this.querySelectorAll('.prev-topic').forEach(btn => {
            btn.addEventListener('click', () => {
                const prevId = btn.dataset.prev;
                this.querySelector(\`[data-topic="\${prevId}"]\`).click();
            });
        });
        ...
        this.querySelectorAll('.option').forEach(option => {
            option.addEventListener('click', () => {
                if (option.closest('.question').classList.contains('answered')) return;
        ...
        // Submit quiz
        this.querySelectorAll('.submit-quiz').forEach(btn => {
            btn.addEventListener('click', () => {
                const quizId = btn.dataset.quiz;
                const quizContainer = this.querySelector('#quiz-' + quizId);
                const questions = quizContainer.querySelectorAll('.question');
                let correct = 0;
                let total = questions.length;
                let allAnswered = true;
                ...
                if (quizId === 'final') {
                    const percentage = Math.round((correct / total) * 100);
                    const passed = percentage >= 80;
                    this.querySelector('#finalResults').classList.add('show');
                    this.querySelector('#finalScoreText').textContent = \`You scored \${correct} out of \${total} (\${percentage}%)\` + (passed ? ' - PASSED!' : ' - Please review and try again.');
                    
                    btn.classList.add('hidden');
                    this.querySelector('.retry-quiz[data-quiz="final"]').classList.remove('hidden');
                    
                    if (passed) {
                        this.querySelector('#finalNavButtons').classList.add('hidden');
                        this.querySelector('#completionSection').classList.add('show');
                        this.querySelector('[data-topic="final"]').classList.add('completed');
                    }
                } else {
                    const resultsDiv = quizContainer.closest('.quiz-section').querySelector('.quiz-results');
                    resultsDiv.classList.add('show');
                    ...
                    btn.classList.add('hidden');
                    btn.closest('.quiz-actions').querySelector('.retry-quiz').classList.remove('hidden');
                    
                    completedTopics.add(parseInt(quizId));
                    this.updateProgress(completedTopics, totalTopics, progressFill, progressText);
                    this.querySelector(\`[data-topic="\${quizId}"]\`).classList.add('completed');
                }
            });
        });

        // Retry quiz
        this.querySelectorAll('.retry-quiz').forEach(btn => {
            btn.addEventListener('click', () => {
                const quizId = btn.dataset.quiz;
                const quizContainer = this.querySelector('#quiz-' + quizId);
                ...
                if (quizId === 'final') {
                    this.querySelector('#finalResults').classList.remove('show');
                    this.querySelector('#completionSection').classList.remove('show');
                    this.querySelector('#finalNavButtons').classList.remove('hidden');
                } else {
                    quizContainer.closest('.quiz-section').querySelector('.quiz-results').classList.remove('show');
                }

                btn.classList.add('hidden');
                btn.closest('.quiz-actions').querySelector('.submit-quiz').classList.remove('hidden');
            });
        });

        // Start final quiz
        const startFinalQuizBtn = this.querySelector('.start-final-quiz');
        if (startFinalQuizBtn) {
            startFinalQuizBtn.addEventListener('click', () => {
                this.querySelector('#finalQuizIntro').classList.add('hidden');
                this.querySelector('#quiz-final').classList.remove('hidden');
                this.querySelector('#finalQuizActions').classList.remove('hidden');
            });
        }
    }

    updateProgress(completedTopics, totalTopics, progressFill, progressText) {
        const percentage = Math.round((completedTopics.size / totalTopics) * 100);
        progressFill.style.width = percentage + '%';
        progressText.textContent = percentage + '% Complete';
    }
...
customElements.define('abo-module-3-ametropia', ABOModule3Ametropia);
